...
yVec=repmat([1:num_labels],m,1)==repmat(y,1,num_labels)%比较并且分类，由0,1组成,5000*10
cost=-yVec.*log(h)-(1-yVec).*log(1-h)%5000*10
J=(1/m)*sum(sum(cost))
...
for t=1:m,
    ...
    d3t=ht-yVecT 
    z2t=[1;Theta1*a1t]%这里加上一行是因为下一层有25个unit+1个bias unit=26行
    d2t=Theta2'*d3t.*sigmoidGradient(z2t)%26*1
    
    delta1 = delta1 + d2t(2:end) * a1t';%为了算grad需要把第一项去除
    delta2 = delta2 + d3t * a2t'%注意最后一层是么有d3t0的
end;

%Theta1_grad = (1 / m) * delta1;
%Theta2_grad = (1 / m) * delta2;

%计算Regularized Neural Networks时要把theta的bias units去掉
